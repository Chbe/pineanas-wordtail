...
const generateFromNameOrEmail = async (
    ...
    username = await setUniqueUsername(username);
...
const generateUsernameFromDisplayname = displayName => {
    ...
    if (displayNameArray.length === 1) {
        ...
        /** E.g. johnsdoe */
        username = `${displayNameArray[0]}${displayNameArray[1].substring(
            0,
            1
        )}${displayNameArray[2]}`.toLowerCase();
    } else {
    }
    return username;
};
...
const prepareDataForUpdate = async (
    ...
    if (!userExist || forceUpdate) {
        if (!anonymous) {
            ...
            saveDocument(`users/${getUid()}`, dataToSave, false, true);
        } else {
            const username = await generateFromNameOrEmail(
                'anonymous',
                false,
